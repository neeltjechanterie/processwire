...
/**
*
* This is an include(\ProcessWire\wire('files')->compile(class for ProcessBlog installWizard($form). It is only run once when finilasing installing Blog,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)))
*
...
class BlogInstallWizard extends ProcessBlog {
	...
	private function createFields() {
		...
		*/
		...
		foreach ($fieldsArray as $fld) {
			...
			$f = new \ProcessWire\Field(); // create new field object
			...
			if (isset($fld['autojoin'])) $f->flags = $f->flags | \ProcessWire\Field::flagAutojoin;
	...
	*/
	private function inContextFieldSettings() {
		...
		$repeaterPage = new \ProcessWire\Page();
	...
	*/
	private function createPages() {
		...
		foreach ($parentPages as $k => $v) {
				...
				$p->parent = $parent;
				$p->title = $v;
				if ($k == 'blog-widgets' || $k == 'blog-settings') $p->addStatus(\ProcessWire\Page::statusHidden);// hidden page - using $k since $v will vary
	...
	*/
	private function extraPageSettings() {
		...
		$f->derefAsPage = 0;// multiple pages (PageArray) - i.e. \ProcessWire\FieldtypePage::derefAsPageArray;
		...
		if ($this->commentsUse == 1) {
					...
					$f->derefAsPage = 1;// single page (PageArray) or boolean false when none selected - i.e. \ProcessWire\FieldtypePage::derefAsPageOrFalse
	...
	*/
	private function createTemplateFiles() {
		...
		if ($this->templateFilesInstall == 1) {
					...
					$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupBlog/BlogInstallWizard.php') . '/template-files/';// source of the template files to copy over [this is 'ProcessBlog/template-files/']
		...
		elseif ($this->templateFilesInstall == 2) {
					...
					$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupBlog/BlogInstallWizard.php') . '/template-files/';
					$destinationpath = $config->paths->templates;// destination: '/site/templates/'
					...
					if($this->demoBlogScriptsInstall) {
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupBlog/BlogInstallWizard.php') . '/scripts/';
					...
					if($this->demoBlogCSSInstall) {
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupBlog/BlogInstallWizard.php') . '/css/';
						...
						$sourcepath = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/MarkupBlog/BlogInstallWizard.php') . '/css/images/';
